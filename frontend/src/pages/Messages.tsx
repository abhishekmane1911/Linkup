...
const Messages = () => {
  ...
  const getOtherParticipant = (conversation: any) => {
    if (conversation.participants && conversation.participants.length > 0) {
      const participant = conversation.participants[0]; // Get the first (and usually only) other participant
      ...
      return {
        ...
        full_name: `${participant.first_name || ''} ${participant.last_name || ''}`.trim() || participant.username,
  ...
  const fetchConversations = async () => {
    try {
      ...
      const transformedConversations: any[] = response.results.map((conv) => ({
        ...
        participants: conv.participants || [], // Keep participants as array from backend
  ...
  const selectConversation = async (conversation: any) => {
    ...
    try {
      ...
      const transformedMessages: DirectMessage[] = response.results.map(
        (msg) => ({
          ...msg,
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const message: DirectMessage = {
        ...
        sent_at: messageResponse.sent_at, // Map created_at to sent_at
      };
          ...
          {isLoading ? (
            ...
            <div className="p-8 text-center text-muted-foreground">
              <div className="mb-4">
                ...
                <p className="text-sm">
                  Visit user profiles and click the message icon to start
                  chatting with people you follow each other.
                </p>
              </div>
            </div>
          ) : (
